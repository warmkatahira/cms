<div class="bg-white rounded-2xl border border-gray-200 shadow-sm w-1/2">
    <button id="toggle_aliases" class="w-full flex items-center justify-between p-6 text-left">
        <p class="text-sm font-semibold text-gray-600">エイリアス情報</p>
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach($aliases as $alias)
                                <span class="inline-flex items-center px-3 py-1 rounded text-xs bg-badge-normal">
                                    ...
                                    @if($alias->users->isNotEmpty())
                                        <span class="ml-1">【{{ $alias->users->map(fn($u) => $u->user_name)->join('・') }}】</span>
                                    @else
                                        <span class="ml-1">【設定なし】</span>
                                    @endif
                                </span>
                            @endforeach
                        </div>
