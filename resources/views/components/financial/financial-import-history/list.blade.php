            ...
            <tbody class="bg-white">
                @foreach($financialImportHistories as $financial_import_history)
                    <tr class="text-left cursor-default whitespace-nowrap hover:bg-table-hover group @if($financial_import_history->message) bg-pink-200 @endif">
                        <td class="py-1 px-2 border">{{ CarbonImmutable::parse($financial_import_history->created_at)->isoFormat('YYYY年MM月DD日(ddd)') }}</td>
                        <td class="py-1 px-2 border">{{ CarbonImmutable::parse($financial_import_history->created_at)->isoFormat('HH時mm分ss秒') }}</td>
                        <td class="py-1 px-2 border text-center">{{ $financial_import_history->import_original_file_name }}</td>
