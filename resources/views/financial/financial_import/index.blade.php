<x-app-layout>
    <div>
        <div class="w-full max-w-2xl">
            <form method="POST" action="{{ route('financial_import.import') }}" enctype="multipart/form-data" id="file_import_form">
                @csrf
                <div class="space-y-4">
                    <div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm transition hover:shadow-md">
                        <div class="flex items-center gap-3 mb-4">
                            <span class="text-sm font-semibold text-gray-700">収支データ</span>
                        </div>
                        <input type="file" id="financial_file" name="financial_file" accept=".csv" class="hidden">
                        <div id="financial_file_area_1"
                            ...
                            <div class="flex flex-col items-center justify-center gap-2 text-gray-400 pointer-events-none">
                                ...
                                <span class="text-sm font-medium" id="financial_file_label_1">クリックまたはドラッグ＆ドロップ</span>
                                <span class="text-xs text-gray-300">.csv のみ対応</span>
                            </div>
                        </div>
                        @error('financial_file')
                ...
                <div class="mt-6 flex justify-end">
                    <button
                        type="button"
                        id="financial_import_enter"
                        class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 active:scale-95 text-white text-sm px-8 py-3 rounded-xl shadow-sm transition-all duration-150"
                    >
