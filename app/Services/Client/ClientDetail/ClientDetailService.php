...
{
    // ─── 共通 ───────────────────────────────────────────────
    private function aliasIds($client)
    {
        return $client->clientAliases->pluck('client_alias_id');
    }
    ...
    // ─── 収支サマリーを取得 ──────────────────────────────────
    public function getSummary($client, FiscalYear $fiscalYear): array
    {
        $summary = MonthlyFinancial::whereIn('client_alias_id', $this->aliasIds($client))
    ...
    // ─── 月次データを取得 ────────────────────────────────────
    public function getMonthly($client, FiscalYear $fiscalYear): array
    {
        $rows = MonthlyFinancial::whereIn('client_alias_id', $this->aliasIds($client))
            ->whereBetween('year_month', [
                $fiscalYear->start_date->format('Y-m-d'),
                $fiscalYear->end_date->format('Y-m-d'),
            ])
